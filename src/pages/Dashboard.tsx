  ...
  useEffect(() => {
    async function fetch() {
      ...
      const [inboxRes, decisionsRes, trackingRes, completedRes, allReqs, recentRes] = await Promise.all([
        ...
        dhub.from('requests').select('category').not('status', 'in', '("consolidated","raw","on_hold")').is('consolidated_into', null),
        dhub.from('requests').select('id, title, category, status, updated_at').not('status', 'in', '("consolidated","raw","on_hold")').is('consolidated_into', null).order('updated_at', { ascending: false }).limit(8),
      ])
